<!-- <template>
  <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-2xl rounded-xl shadow-lg flex flex-col max-h-[90vh]">
      ...
      <div class="p-4 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        <div v-for="img in images" :key="img" class="border rounded-lg p-2 cursor-pointer hover:ring-2 ring-sky-500">
          <div class="aspect-square bg-gray-200 rounded mb-1"></div>
          <p class="text-xs text-center">{{ img }}</p>
        </div>
      </div>
